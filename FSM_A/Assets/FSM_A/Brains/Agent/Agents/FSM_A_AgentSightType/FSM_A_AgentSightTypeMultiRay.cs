...
public class FSM_A_AgentSightTypeMultiRay
{
    ...
    public void DrawGizmos(Transform _origin, float _height, float _range, float _angle, LayerMask _targetLayer, LayerMask _obstacleLayer)
    {
        for (int i = -(int)_angle/2; i <_angle/2; i++)
        {
            Ray _toTargetRay = new Ray(_origin.position  + Vector3.up * _height, Quaternion.Euler(Mathf.Sin(Time.time) * 20, i, 0) * _origin.forward);
            RaycastHit _hit;
            bool _hitTarget = Physics.Raycast(_toTargetRay, out _hit, _range, _targetLayer);
            Gizmos.color = !Util.HitObstacleBetweenTarget(_hit, _origin.position , (Quaternion.AngleAxis(i, Vector3.up)*_origin.forward),_range, _obstacleLayer)? Color.green : Color.red;
            Gizmos.DrawRay(_toTargetRay.origin, _toTargetRay.direction * _range);
